...
public class AnalizadorSintactico {
	...
	private void nextToken() throws Exception {
		...
		if(ventana.size() > 0) {
			...
			if(comentario != "") {
				...
				gestorSal.emite(" { " + comentario + " } \n"); // suponemos que todos los comentarios son de varias lineas
			}
		}
	}
	...
	private ExpresionTipo literal() throws Exception {
		if(token.esIgual(TipoToken.CARACTER)){
			...
			String sal = ((Integer)token.getAtributo() == 27) ? "false" : "true";
			gestorSal.emite(sal);
			nextToken();
	...
	private void programa() throws Exception {
		parse.add(1);
		gestorSal.setModo(modoSalida.PROGRAMA);
		gestorSal.emite("PROGRAM Ejemplo;\n\n");
		gestorSal.emite("uses: CRT;\n");
		gestorSal.emitirRes();
		libreria();
		resto_programa();
		if (!token.esIgual(TipoToken.EOF)) {
			...
			gestorSal.emitirConsts();
			gestorSal.emitirTipos();
			gestorSal.emitirVars();
			gestorSal.emitirRes();
			gestorSal.emitirPrincipal();
			gestorSal.emitirRes();
		}
	}
	...
	private ExpresionTipo cosas() throws Exception {
		...
		if(!token.esIgual(TipoToken.EOF)) {
			if(token.esIgual(TipoToken.PAL_RESERVADA, 9 /*const*/)){
				...
				gestorSal.setModo(modoSalida.CONSTANTE);
				nextToken();
				if(esTipo(token)){
				...
				tipoRetorno = null;
				nextToken();
				nombreFuncion = ((EntradaTS)token.getAtributo()).getLexemaTrad(); 
				if(!nombreFuncion.equals("main"))
					gestorSal.emite("PROCEDURE");
				...
				if(token.esIgual(TipoToken.IDENTIFICADOR)) {
					...
					EntradaTS ent = (EntradaTS) token.getAtributo();
					if(!nombreFuncion.equals("main"))
						gestorSal.emite(ent.getLexemaTrad());
					nextToken();
					if(token.esIgual(TipoToken.SEPARADOR, Separadores.ABRE_PARENTESIS)){
	...
	*/
	...
	private ExpresionTipo cosas2(ExpresionTipo COSAS2_tipo_h,ExpresionTipo tipo_id, EntradaTS entrada) throws Exception {
		if(token.esIgual(TipoToken.SEPARADOR,Separadores.ABRE_PARENTESIS)) {
			...
			parse.add(16);
			if(!gestorSal.getModo().equals(modoSalida.PRINCIPAL))
			...
			parse.add(17);
			INICIALIZACION_tipo_h=inicializacion(COSAS2_tipo_h,entrada);
			DECLARACIONES_tipo_h=declaraciones(COSAS2_tipo_h);
			if(!token.esIgual(TipoToken.SEPARADOR,Separadores.PUNTO_COMA)) {
	...
	*/
	...
	private ExpresionTipo inicDim3(ExpresionTipo tipo_h, int num_comp, int longitud, String nombreArray) throws Exception {
		if(!token.esIgual(TipoToken.EOF)){
			...
			if(LITERAL_TIPOSIMPLE != null){
				...
				INIC_DIM2_tipo_s=inicDim2(tipo_h,num_comp,longitud,nombreArray);
	...
	private ExpresionTipo inicDim5(ExpresionTipo tipo_h, int longitud, String nombreArray) throws Exception {
		if(token.esIgual(TipoToken.SEPARADOR, Separadores.COMA)){
			...
			nextToken();
			ExpresionTipo INIC_DIM2_tipo_s,INIC_DIM5_tipo_s;
			INIC_DIM2_tipo_s=inicDim2(tipo_h,1,longitud,nombreArray);
			INIC_DIM5_tipo_s=inicDim5(tipo_h,longitud,nombreArray);
			if(INIC_DIM2_tipo_s.getTipoBasico()!=TipoBasico.error_tipo && INIC_DIM5_tipo_s.getTipoBasico()!=TipoBasico.error_tipo){
				return ExpresionTipo.getVacio();
			}
	...
	private ExpresionTipo inicializacion(ExpresionTipo tipo_h,EntradaTS entrada) throws Exception {
		if(token.esIgual(TipoToken.OP_ASIGNACION)) {
			...
			parse.add(39);
			hayInic=true;
			if(gestorSal.getModo()==modoSalida.PROGRAMA)
				gestorSal.setModo(modoSalida.PRINCIPAL);
			...
			String s = gestorSal.getBufferExpString();
			...
			if(token.esIgual(TipoToken.NUM_ENTERO )){
				...
				if(token.esIgual(TipoToken.SEPARADOR, Separadores.CIERRA_CORCHETE)){
					...
					compilador.analizadorSemantico.Vector v = new compilador.analizadorSemantico.Vector((Integer)(token_aux.getAtributo()), tipo_h);
					entrada.setTipo(v);
					nextToken();
	...
	/**
	 *  94. RESTO_WHILE --> (EXPRESSION) CUERPO2
	 *  {
	...
	private ExpresionTipo resto_while() throws Exception {
		...
		if(token.esIgual(TipoToken.SEPARADOR,Separadores.ABRE_PARENTESIS)){
			parse.add(94);
			nextToken();
			 EXPRESSION_tipo = expression(false,false,false);
			if(token.esIgual(TipoToken.SEPARADOR,Separadores.CIERRA_PARENTESIS)){			
				nextToken();
				gestorSal.emite("do\n");
		...
		if(token.esIgual(TipoToken.PAL_RESERVADA,6 /*case*/)) {
			...
			if(LITERAL_tipo != null)
			{
				if(token.esIgual(TipoToken.SEPARADOR,Separadores.DOS_PUNTOS)){
					...
					lexico.desactivaModo(modo.Declaracion);
					nextToken();
					if(ExpresionTipo.sonCompLog(EXPRESSION_tipo, LITERAL_tipo, OpLogico.AND) == null){
		...
		if(token.esIgual(TipoToken.SEPARADOR,Separadores.ABRE_PARENTESIS)){
			...
			if(token.esIgual(TipoToken.SEPARADOR,Separadores.CIERRA_PARENTESIS)){
				...
				if(CUERPO2_tipo.getTipoBasico() != TipoBasico.error_tipo &&
					...
					EXPRESSION_tipo.getTipoBasico() != TipoBasico.error_tipo) {
						ExpresionTipo tipo = ExpresionTipo.getVacio();
						gestorTS.cierraBloque();
	...
	private ExpresionTipo primary_expression() throws Exception {
		...
		if (token.esIgual(TipoToken.SEPARADOR, Separadores.ABRE_PARENTESIS)) {
			...
			parse.add(147);
			nextToken();
			tipo = expression(false,false,false);
			if (token.esIgual(TipoToken.SEPARADOR, Separadores.CIERRA_PARENTESIS)) {
				gestorSal.emite(") ");
				nextToken();
			} else {
			...
			parse.add(145);
			gestorSal.emite("{ No se puede traducir a pascal el 'this' }\n");
			nextToken();
	...
	private ExpresionTipo postfix_expression() throws Exception {
		...
		if (token.esIgual(TipoToken.PAL_RESERVADA, 63)) {
			...
			if (token.esIgual(TipoToken.SEPARADOR, Separadores.ABRE_PARENTESIS)) {
				nextToken();
				ExpresionTipo aux1 = expression(false,false,false);
				if (token.esIgual(TipoToken.SEPARADOR, Separadores.CIERRA_PARENTESIS)) {
					nextToken();
					ExpresionTipo aux2 = resto_postfix_exp(aux1);
					if(aux1.equals(TipoBasico.error_tipo) || aux2.equals(TipoBasico.error_tipo)){
						tipo = ExpresionTipo.getError();
					} else {
						tipo = aux2;
	...
	private ExpresionTipo resto_postfix_exp(ExpresionTipo exp) throws Exception {
		...
		if (token.esIgual(TipoToken.SEPARADOR, Separadores.ABRE_CORCHETE)) {
			...
			parse.add(155);
			nextToken();
			ExpresionTipo aux = expression(false,false,false);
			if((!exp.equals(TipoNoBasico.vector)) && (!exp.equals(TipoNoBasico.cadena))) {
				tipo = ExpresionTipo.getError();
				gestorErr.insertaErrorSemantico(linea, columna, "Aqui no vale lo de los [ ]");
			}else{
	...
	private ExpresionTipo resto_postfix_exp3(ExpresionTipo exp) throws Exception {
		...
		if (token.esIgual(TipoToken.OP_LOGICO,OpLogico.SOBRERO)) {	
			...
			if (token.esIgual(TipoToken.PAL_RESERVADA, 16)) {
				...
				if (token.esIgual(TipoToken.SEPARADOR, Separadores.ABRE_PARENTESIS)) {
					nextToken();
					aux =  expression(false,false,false);
					if (token.esIgual(TipoToken.SEPARADOR, Separadores.CIERRA_PARENTESIS)) {
						nextToken();
					} else {
						gestorErr.insertaErrorSintactico(linea, columna,"Falta el separador \")\"");
	...
	private ExpresionTipo initializer_list() throws Exception{
		...
		if ( !aux1.getTipoBasico().equals("error_tipo")) {
			...
			gestorErr.insertaErrorSemantico(linea, columna, "LALALALALALAALALA");
			return ExpresionTipo.getError();
		}
	}
	...
	**/
	...
	private ExpresionTipo resto_cast() throws Exception{
		...
		if(tipo != null){
			if(token.esIgual(TipoToken.SEPARADOR,Separadores.CIERRA_PARENTESIS)){
				nextToken();
				parse.add(139);
				ExpresionTipo aux = cast_expression();
				gestorSal.emite(")");
				if(!tipo.equals(TipoBasico.error_tipo)){
					tipo = aux;
				}
	 ...
	 * 							  { BIS }
	 * 216. RESTO-RELATIONAL → >= SHIFT_EXPRESSION
	 * 							  { BIS }
	 * 217. RESTO-RELATIONAL → <= SHIFT_EXPRESSION
	 * 							  { BIS }
	...
	*/
	...
	private ExpresionTipo resto_conditional(ExpresionTipo tipo_h) throws Exception {
		if(token.esIgual(TipoToken.SEPARADOR, Separadores.INTEROGACION)){
			...
			nextToken();
			ExpresionTipo aux1 = expression(false,false,true);
	 ...
	 * @throws Exception 
	 */
	private ExpresionTipo expression(boolean retornoMain, boolean incFor,boolean expInterna) throws Exception {
		parse.add(246);
		modoSalida modo_anterior = gestorSal.getModo();
		gestorSal.setModo(modoSalida.EXPRESION);
		ExpresionTipo aux1 = assignment_expression();
	...
	private ExpresionTipo expressionOpt(boolean retornoMain, boolean incFor) throws Exception {
		if(primeroDeExpression()) {
			parse.add(249);
			return expression(retornoMain,incFor,false);
		}
		else{
			parse.add(250);
			return ExpresionTipo.getVacio();
